...
func InsertCredential(db *sql.DB, cred structs.Credential) error {
	...
	now := time.Now()
	_, err := db.Exec(query, cred.Username, cred.Password, cred.Description, tagsJSON, now, now)
	if err != nil {
...
func GetCredential(db *sql.DB, id int) (*structs.Credential, error) {
	...
	err := db.QueryRow(query, id).Scan(
		...
		&cred.Description, &cred.Tags, &cred.CreatedAt, &cred.UpdatedAt,
	)
...
func UpdateCredential(db *sql.DB, id int, cred structs.Credential) error {
	...
	now := time.Now()
	result, err := db.Exec(query, cred.Username, cred.Password, cred.Description, cred.Tags, now, id)
	if err != nil {
		return response.WrapError(err, response.ErrDatabaseConnection)
	}
